...
com_st = L(1,1);
for i = 1 : N
    if L(1 : km, i) < abs(com_st * 10^(-5))
        c1 = i-1;
        break;
    end
end
for j = i : N
    if L(km+1 : 2*km, j) < abs(com_st * 10^(-5))
      c2 = j - 1;
    end
end
...
L51 = L(4*km+1 : 5*km, 1 : c1);
L52 = L(4*km+1 : 5*km, c1+1 : c2);
...
gamma = (L51*P1' + L52*P2')*sqrt((P1*P1' + P2*P2'));
...
DB1 = [eye(p) zeros(p,n); zeros(kp) sU1];
