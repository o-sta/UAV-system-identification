...
% システム同定に必要なデータを取得します。
%--------------------------------------------------------------------------
number_of_input = 1;                    % 入力の数 (ft tx ty tz)
number_of_output = 2;                   % 出力の数 (z w phi theta psi p q r)
number_of_state_variables = 2;          % 状態変数の数(9が一番良い)
...
%--------------------------------------------------------------------------
mls_ft = mf_createMLS(MLS_degree, 1, min_value(1), max_value(1), length_of_time, continuous_signal_ts);

%--------------------------------------------------------------------------
% ● シミュレーション及びCL-MOESP実行
%--------------------------------------------------------------------------
sim('block/z_sysid', end_time);
data_discrete = getsampleusingtime(data_discrete, start_time, end_time);
reference_data = data_discrete.data(:, 27)'; % M系列信号(tx ty tzに重畳)
input_data = data_discrete.data(:, 13)';     % 入力データ行列(tx ty tz)
output_data = horzcat(data_discrete.data(:, 3), data_discrete.data(:, 6))'; % 出力データ行列(phi theta psi p q r)
...
end_time = start_time + length_of_time; % 同定終了時間（シミュレーション終了時間）
% M系列信号を同定時とは違うものにする
mls_ft = mf_createMLS(MLS_degree, 2, min_value(1), max_value(1), length_of_time, continuous_signal_ts);
...
fit_percent = [0 0 mf_fit(data_est_discrete.Data(:,3), data_est_discrete.Data(:,28));
0 0 mf_fit(data_est_discrete.Data(:,3), data_est_discrete.Data(:,29));
0 0 0;
0 0 0;]
...
% matrix_C = est_c_C * est_c_C^(-1);
% matrix_D = est_c_D;
save('data/estimated_matrix_z','matrix_A','matrix_B','matrix_C','matrix_D');
