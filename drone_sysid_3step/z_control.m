...
% ● システム同定で推定した行列を読み込む
%--------------------------------------------------------------------------
load data/estimated_matrix_z; % matrix_A, matrix_B, matrix_C, matrix_D を読み込む
...
% 出力と入力に重みをつける
%--------------------------------------------------------------------------
weight_U = diag([1]);       % 入力に関する重み [ft tx ty tz]
weight_Y = diag([100 1]); % 出力に関する重み [z w phi theta psi p q r]
% 上記で定義した入出力の重みから
